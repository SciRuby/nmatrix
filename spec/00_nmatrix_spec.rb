...
describe NMatrix do
      ...
      [:object, :int64].each do |dtype|
        ...
        context c do
          ...
          it "allows storage-based iteration of matrices" do
            pending("not yet implemented for sparse matrices for NMatrix-JRuby") if jruby? && (storage_type != :dense)
  ...
  it "should return an enumerator when each is called without a block" do
    ...
    begin
      atans = []
      loop { atans << Math.atan2(*enums.map(&:next)) }
