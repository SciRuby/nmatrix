...
// be careful that each rb_raise should have a free_data before it here.
...
static int actually_parse(FILE* p_file, csv_parser* p_parser) {
  ...
  while(!feof(p_file))
  {
    ...
    if(bytes_read != BUFFERSIZE && ferror(p_file)) {
      free_data(p_file, p_parser);
      rb_raise(rb_eIOError, "fread error\n");
      return -1;
    }
    ...
    if(parser_processed != bytes_read) { // error handling
      free_data(p_file, p_parser);
      rb_raise(rb_eRuntimeError, "malformed csv file. csv_parse: %s\n.",
  ...
  if(result != 0) {
    free_data(p_file, p_parser);
    rb_raise(rb_eRuntimeError, "malformed csv file. csv_fini: %s\n.",
                csv_strerror(csv_error(p_parser)));
    return -1;
  }
