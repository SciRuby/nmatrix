class NMatrix
  ...
  def atan2(other, scalar = false)
    result = create_dummy_nmatrix
    result.s = if scalar
      ArrayRealVector.new MathHelper.atan2Scalar(other, @s.toArray)
    else
      result.s = if other.is_a? NMatrix
        ArrayRealVector.new MathHelper.atan2(other.s.toArray, @s.toArray)
      else
        ArrayRealVector.new MathHelper.atan2Scalar2(other, @s.toArray)
      end
    end
    result
  end
  ...
  def ldexp(other, scalar = false)
    result = create_dummy_nmatrix
    result.s = if scalar
      ArrayRealVector.new MathHelper.ldexpScalar(other, @s.toArray)
    else
      result.s = if other.is_a? NMatrix
        ArrayRealVector.new MathHelper.ldexp(other.s.toArray, @s.toArray)
      else
        ArrayRealVector.new MathHelper.ldexpScalar2(other, @s.toArray)
      end
    end
    result
  end
  ...
  def hypot(other, scalar = false)
    result = create_dummy_nmatrix
    result.s = if scalar
      ArrayRealVector.new MathHelper.hypotScalar(other, @s.toArray)
    else
      result.s = if other.is_a? NMatrix
        ArrayRealVector.new MathHelper.hypot(other.s.toArray, @s.toArray)
      else
        ArrayRealVector.new MathHelper.hypotScalar(other, @s.toArray)
      end
    end
  ...
  def log(val = :natural)
    result = create_dummy_nmatrix
    result.s = if val == :natural
      @s.copy.mapToSelf(Log.new)
    else
      ArrayRealVector.new MathHelper.log(val, @s.toArray)
    end
    result
  end
