class NMatrix
  def get_slice(dim, args, shape_array)
    ...
    (0...dim).each do |r|
      ...
      if argc - t + r < dim && shape_array[r] == 1
        ...
        v_ = v.to_i.to_int
        slice[:coords][r] = if v_ < 0 # checking for negative indexes
          shape_array[r] + v_
        else
          v_
        end
        ...
        slice[:lengths][r] = if end_ < 0
          shape_array[r] + end_ - slice[:coords][r] + (excl ? 0 : 1)
        else
          end_ - slice[:coords][r] + (excl ? 0 : 1)
        end
  ...
  def xslice_ref(args)
    ...
    if dim < args.length
      ...
      slice = get_slice(@dim, args, @shape)
      stride = get_stride(self)
      result = if slice[:single]
        if @dtype == :object
          @s[dense_storage_get(slice, stride)]
        else
          @s.getEntry(dense_storage_get(slice, stride))
        end
      else
        dense_storage_ref(slice, stride)
      end
    end
    result
  end
  ...
  def dense_storage_get(slice, stride)
    if slice[:single]
      dense_storage_pos(slice[:coords], stride)
    else
      shape = @shape.dup
      (0...@dim).each do |i|
      ...
      arr = if @dtype == :object
        Java::boolean[s.length].new
      else
        Java::double[s.length].new
      end
      (0...s.length).each do |i|
        arr[i] = s[i]
      end
      result.s = if @dtype == :object
        arr
      else
        ArrayRealVector.new(arr)
      end
