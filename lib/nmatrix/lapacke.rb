...
class NMatrix
      ...
      def geev(matrix, which = :both)
        ...
        unless matrix.complex_dtype?
          ...
          unless complex_indices.empty?
            ...
            eigenvalues += imag_eigenvalues * Complex(0.0, 1.0)
  ...
  def getrf!
    ...
    ipiv = NMatrix::LAPACK.lapacke_getrf(:row, shape[0], shape[1], self, shape[1])
  ...
  def potrf!(which)
    raise(StorageTypeError, "LAPACK functions only work on dense matrices") unless dense?
    raise(ShapeError, "Cholesky decomposition only valid for square matrices") unless dim == 2 && shape[0] == shape[1]

    NMatrix::LAPACK.lapacke_potrf(:row, which, shape[0], self, shape[1])
  end
  ...
  def geqrf!
    ...
    tau = NMatrix.new([shape.min, 1], dtype: dtype)
    NMatrix::LAPACK.lapacke_geqrf(:row, shape[0], shape[1], self, shape[1], tau)
  ...
  def ormqr(tau, side = :left, transpose = false, c = nil)
    raise(StorageTypeError, "LAPACK functions only work on dense matrices") unless dense?
    raise(TypeError, "Works only on floating point matrices, use unmqr for complex types") if complex_dtype?
    raise(TypeError, "c must have the same dtype as the calling NMatrix") if c && (c.dtype != dtype)

    # Default behaviour produces Q * I  = Q if c is not supplied.
    result = c ? c.clone : NMatrix.identity(shape[0], dtype: dtype)
    NMatrix::LAPACK.lapacke_ormqr(:row, side, transpose, result.shape[0], result.shape[1], tau.shape[0], self, shape[1], tau, result, result.shape[1])
  ...
  def unmqr(tau, side = :left, transpose = false, c = nil)
    raise(StorageTypeError, "ATLAS functions only work on dense matrices") unless dense?
    raise(TypeError, "Works only on complex matrices, use ormqr for normal floating point matrices") unless complex_dtype?
    raise(TypeError, "c must have the same dtype as the calling NMatrix") if c && (c.dtype != dtype)

    # Default behaviour produces Q * I  = Q if c is not supplied.
    result = c ? c.clone : NMatrix.identity(shape[0], dtype: dtype)
    NMatrix::LAPACK.lapacke_unmqr(:row, side, transpose, result.shape[0], result.shape[1], tau.shape[0], self, shape[1], tau, result, result.shape[1])
